...
标准件计数和缺陷件投票均要求检测框完整位于横向`120～520`像素有效区内。工件从右向左运动，刚进入右侧或即将离开左侧时，只在MJPEG画面显示检测框和`WAIT ROI`提示，不计数、不投票、不发布抓取坐标。缺陷件从左侧离开有效区时会清空尚未完成的投票，避免穿线孔因凹槽遮挡产生的错误分类累积成停带和抓取指令。视频中两条黄色竖线及`ACTIVE ROI`文字用于标识有效区，可根据实机视角调整两个边界参数。
...
'quexianketi':  [180, 12, 72, 75, 89, 29]
```

缺陷件投放姿态原第1关节为222度，超出当前`Arm_Lib`对第1关节规定的
0～180度范围，底层会打印参数超限后直接忽略整条六关节命令。当前修正为
180度，并在每次投放前调用`validate_place_joint()`检查六个关节范围；任何
...
传送带控制板采用非阻塞连续采样消抖：每1 ms读取一次PA0、PA1，连续高15 ms才确认命令；执行后必须连续低20 ms才重新允许下一次触发，因此单个50 ms脉冲只执行一次。PA0停止和PA1启动同时有效时停止优先，执行启动前还会确认PA0为低。该逻辑能够过滤短时数字毛刺，但不能代替共地、电源去耦、信号线隔离和必要的硬件RC或施密特滤波。
