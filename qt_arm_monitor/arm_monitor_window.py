...
_SSH_USER = os.environ.get('DOFBOT_SSH_USER', 'jetson')
_SSH_PASSWORD = os.environ.get('DOFBOT_SSH_PASSWORD', '')
...
class ArmMonitorWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        saved_ip = self._settings.value('last_ip', '10.182.135.194')
        if saved_ip in ('10.229.42.172', '10.178.59.172', '192.168.8.88'):
            saved_ip = '10.182.135.194'
    ...
    def _ssh_quick_cmd(self, cmd):
        ...
        def _run():
            try:
                ...
                c.connect(host, username=_SSH_USER, password=_SSH_PASSWORD, timeout=5)
                c.exec_command(cmd)
                time.sleep(0.3)
                c.close()
            except Exception:
    ...
    def _toggle_ssh(self):
        if self.ssh_worker and self.ssh_worker.isRunning():
            ...
            if _IS_LOCAL:
                ...
                if not host:
                    return
                self.ssh_worker = SSHLogWorker(host, _SSH_USER, _SSH_PASSWORD, log_files)
                self.ssh_worker.log_signal.connect(self._on_ssh_log)
                self.ssh_worker.status_signal.connect(self._on_ssh_status)
                self.ssh_worker.start()
    ...
    def _parse_log_line(self, line):
        ...
        # Standard parts are counted by the detector but pass through without
