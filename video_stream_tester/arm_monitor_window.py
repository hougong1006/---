...
class CameraStreamWorker(QThread):
    """Background thread: read MJPEG stream from frame_streamer HTTP server."""
    frame_signal = pyqtSignal(object)  # QImage
    status_signal = pyqtSignal(str)
            ...
            try:
                ...
                while self._running:
                    ...
                    while True:
                        ...
                        b = buf.find(b'\xff\xd9', a + 2 if a >= 0 else 0)  # JPEG EOI
                        if a < 0 or b < 0:
                            break
                        jpg_data = buf[a:b + 2]
                        buf = buf[b + 2:]
                        qimg = QImage()
                        qimg.loadFromData(jpg_data)
                        if not qimg.isNull():
...
class StatusPollWorker(QThread):
    """Background thread: poll /status endpoint for arm joint angles."""
    joints_signal = pyqtSignal(list)  # [float x6]
        ...
        while self._running:
            try:
                ...
                if 'joints' in data and len(data['joints']) >= 6:
                    self.joints_signal.emit(data['joints'][:6])
            except Exception:
                pass
            time.sleep(0.5)
...
class ArmMonitorWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        saved_ip = self._settings.value('last_ip', '192.168.0.104')
        if saved_ip in ('10.229.42.172', '10.178.59.172',
                        '10.182.135.172', '10.182.135.194', '192.168.8.88'):
            saved_ip = '192.168.0.104'
        ...
        self.btn_video_only = QPushButton("只连接视频")
        self.btn_video_only.setFont(QFont(_FONT, 9))
        ...
        """)
        self.btn_video_only.clicked.connect(self._toggle_video_only)
        banner_layout.addWidget(self.btn_video_only)
        ...
        self.camera_label = QLabel("摄像头显示区域")
        ...
        self._system_stopped = False  # True after user clicks Stop; prevents SSH reconnect from restarting camera
        self._log_count = 0
    ...
    def _on_launch_nodes_ready(self):
        ...
        self.add_log("[SYS] Nodes ready — restarting camera & log tail")
    ...
    def _on_ssh_status(self, status):
        ...
        if status == 'connected':
            ...
            self._connect_time = time.time()
            self._uptime_timer.start()
            # Auto-start camera stream (system may already be running)
            if not self._system_stopped and not self._video_only_mode:
                self._start_camera_stream()
    ...
    def _toggle_video_only(self):
        ...
        if not host:
            self.add_log("[VIDEO] Device IP is empty")
    ...
    def _start_camera_stream(self, video_only=False):
        ...
        self._stop_camera_stream()  # clean up any existing
        self._video_only_mode = bool(video_only)
        if _IS_LOCAL:
            host = '127.0.0.1'
        else:
            host = self.ip_input.text().strip()
        # Start camera MJPEG stream
    ...
    def _stop_camera_stream(self):
        """Stop the camera stream and status polling workers."""
        if self._status_worker and self._status_worker.isRunning():
            self._status_worker.stop()
            self._status_worker.wait(2000)
            self._status_worker = None
        if self._cam_worker:
            # Disconnect signal FIRST so stale frames won't update UI
            try:
                self._cam_worker.frame_signal.disconnect(self._on_camera_frame)
                self._cam_worker.status_signal.disconnect(self._on_camera_status)
            except Exception:
                pass
    ...
    def _on_camera_frame(self, qimg):
        """Display a new camera frame in the UI."""
        if not self._cam_worker:
            return  # Ignore stale frames after camera was stopped
        if not self._video_frame_seen:
            self._video_frame_seen = True
            self.add_log(
                f"[VIDEO] First frame received: {qimg.width()}x{qimg.height()}")
    ...
    def _on_camera_status(self, status):
        """Show video-only connection diagnostics in the existing log panel."""
        if status.startswith("connecting:"):
            self.add_log("[VIDEO] Connecting to " + status[11:])
        elif status == "connected":
            self.add_log("[VIDEO] HTTP connection established, waiting for JPEG frames")
        elif status.startswith("error:"):
            error = status[6:]
            self.add_log("[VIDEO] Connection error: " + error)
            if not self._video_frame_seen:
                self.camera_label.setText("视频连接失败\n" + error)
