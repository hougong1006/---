...
启动脚本会先检查六关节自检是否已经结束，并自动清理上一次分拣或独立视频模式遗留的进程；随后完成GPIO引脚复用配置，通过BCM13发送50 ms启动脉冲，再启动各ROS 2节点。每个节点运行在独立进程组中，任一节点失败会立即回滚。停止脚本还会使尚在初始化的旧启动流程失效，防止点击停止后旧脚本继续拉起后续节点。Windows可视化界面的一键启停调用同一组脚本。启动前应确保传送带检测区域内没有待检工件，防止节点初始化期间工件未经检测直接通过。
