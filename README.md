...
- `place_pose_capture`：停止分拣后手动示教并读取缺陷件投放姿态的工具。
...
- 当前设备IP：`192.168.50.104`（以太网直连）；无线备用地址仍为`10.226.252.172`。
...
- 设备有效坐标补偿：X=-0.007754、Y=-0.04541、Z=-0.0199。
...
停止脚本优先调用分拣节点的`/stop_conveyor`服务，由已经占用BCM6的节点发送150 ms停止脉冲，再终止ROS 2节点。若设备仍运行不支持该服务的旧分拣节点，脚本会在同一次停止流程中释放GPIO后自动补发停止脉冲。Windows可视化界面的停止按钮调用相同脚本。

启动脚本会先检查六关节自检是否已经结束，并自动清理上一次分拣或独立视频模式遗留的进程；随后完成GPIO引脚复用配置，通过BCM13发送150 ms启动脉冲，再启动各ROS 2节点。每个节点运行在独立进程组中，任一节点失败会立即回滚。停止脚本还会使尚在初始化的旧启动流程失效，防止点击停止后旧脚本继续拉起后续节点。Windows可视化界面的一键启停调用同一组脚本。启动前应确保传送带检测区域内没有待检工件，防止节点初始化期间工件未经检测直接通过。
...
需要在不启动分拣系统的情况下单独返回检测等待位置时，执行`chushi`。该命令只控制机械臂移动到`[89, 56, 94, -36, 90, 30]`并校验反馈，不操作传送带、相机或YOLO；如果检测到其他机械臂控制程序正在运行，会拒绝执行。
